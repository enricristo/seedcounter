...
| 2.13 | **Gerador de cena sintética com verdade** | Enrico, 13/09 (esteira virtual, parte estática) | compor recortes anotados (`image-crop.ts`) sobre um fundo (`background.ts`) em posições conhecidas → imagem + verdade exata (contorno, classe, posição). É o que a memória pedia para qualquer "modo avançado"; barateia todo "medir se" deste degrau (2.5, 2.9, 2.10, 2.11, 2.12) e dá à orquídea a verdade de referência que ela não tem. **Não prova** sombra, encosto físico nem desfoque — recorte colado não é foto | função pura `comporCena(recortes, fundo, layout) → {imagem, verdade}`; teste: onda sobre cena de 20 recortes recupera ≥ 95% com IoU > 0,8; uma cena de soja e uma de orquídea no `docs/datasets/` |
...
**Ordem sugerida:** 2.8 (TIFF, destrava o scanner) → 2.13 (cena sintética: barateia toda medição que vem depois) → 2.11 (assim que a Task 7 fechar) → 2.1 → 2.2 → 2.9/2.10 (depois de detalhar com o Enrico) → 2.3 → 2.4 → 2.5 → 2.6 → 2.12 (com a medição na soja). O 2.7 espera a conferência humana.
...
| 3.8 | **Esteira: contagem em fluxo** — dataset real de trigo em esteira (AVI + frames, com *foreign matter*) e a **esteira virtual** (2.13 animada: deslocar a cena em x por quadro) | Enrico, 13/09 | o problema não é segmentar o quadro, é **não contar a mesma semente duas vezes**: rastreio 1D (prever `x + v·dt`, associar ao mais próximo) + linha de contagem; *foreign matter* como classe própria na taxonomia. A virtual mede contagem e rastreio com verdade exata; o AVI real decide encosto, borrão e impureza. Navegador não abre AVI → frames extraídos (TIFF → 2.8). Depende de Task 7 (inferência fora da thread) e de 2.13 |
