...
export function ExportCard({ icon, title, desc, onClick, disabled = false }: ExportCardProps) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="flex flex-col items-start text-left p-4 rounded-xl border border-neutral-200 dark:border-zinc-800 hover:border-neutral-300 dark:hover:border-zinc-700 hover:shadow-md transition-all bg-surface-1 group active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none"
    >
